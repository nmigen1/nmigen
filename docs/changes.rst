...
* Update ``import nmigen as nm`` :ref:`explicit prelude imports <lang-prelude>` to be ``import nmigen as am``, and adjust the code to use the ``am.*`` namespace.
...
* Added: If a compatible Yosys version is not installed, :mod:`back.verilog` will fall back to the `nmigen-yosys <https://github.com/nmigen-lang/nmigen-yosys>`_ PyPI package. The package can be :ref:`installed <install>` as ``nmigen[builtin-yosys]`` to ensure this dependency is available.
